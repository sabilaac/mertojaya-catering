			...
			<?php foreach ($data_list as $i => $item) : ?>
				<a href="<?= base_url() . 'package?url=' . $item->url ?>">
					<div class="package-showcase-item category_<?= $item->category_id ?>">
						<div class="thumbnail">
							<?php if ($item->photo_1) : ?>
								<img src="<?= base_url() . 'image/' . $item->photo_1_uuid ?>"
									 alt="Thumbnail"/>
							<?php elseif ($item->photo_2) : ?>
								<img src="<?= base_url() . 'image/' . $item->photo_2_uuid ?>"
									 alt="Thumbnail"/>
							<?php elseif ($item->photo_3) : ?>
								<img src="<?= base_url() . 'image/' . $item->photo_3_uuid ?>"
									 alt="Thumbnail"/>
							<?php elseif ($item->photo_4) : ?>
								<img src="<?= base_url() . 'image/' . $item->photo_4_uuid ?>"
									 alt="Thumbnail"/>
							<?php endif; ?>
